    ...
    <div class="auth-split">
      ...
      <div class="auth-left">
        ...
        <div class="auth-left-content">
          ...
          <p class="auth-left-sub">Connectez-vous pour continuer à commander vos produits frais, suivre vos courses et échanger avec votre coursière.</p>
          ...
          <div class="auth-features">
            <div class="auth-feature">
              ...
              <div><p>Produits frais</p><span>Sélectionnés directement au marché</span></div>
            </div>
            <div class="auth-feature">
              <span class="auth-feature-icon">
              ...
              <div><p>Paiement sécurisé</p><span>Mobile Money sécurisé</span></div>
            </div>
            <div class="auth-feature">
              <span class="auth-feature-icon">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8Z"/></svg>
              </span>
              <div><p>Livraison rapide</p><span>Une coursière proche de vous</span></div>
            </div>
          </div>
          ...
          <form @submit.prevent="seConnecter">
            ...
            <div class="form-group">
              ...
              <div class="password-field">
                ...
                <input v-model="form.motDePasse" :type="showPassword ? 'text' : 'password'" placeholder="••••••••" required />
            ...
            <label class="remember-check">
              <input type="checkbox" v-model="seSouvenir" />
...
const DERNIER_IDENTIFIANT_KEY = 'dernierIdentifiant'
...
const seSouvenir = ref(true)

onMounted(() => {
  const dernier = localStorage.getItem(DERNIER_IDENTIFIANT_KEY)
  if (dernier) form.value.identifiant = dernier
})
...
async function seConnecter() {
  ...
  try {
    ...
    if (seSouvenir.value) {
      localStorage.setItem(DERNIER_IDENTIFIANT_KEY, form.value.identifiant)
    } else {
      localStorage.removeItem(DERNIER_IDENTIFIANT_KEY)
    }
...
.auth-left-bg {
  ...
  background: url('https://images.unsplash.com/photo-1585540083814-ea6ee8af9e4f?w=900&h=1100&fit=crop&q=75') center/cover;
}
.auth-left-overlay {
  position: absolute; inset: 0;
...
.auth-card h2 { font-size: 26px; font-weight: 800; margin-bottom: 6px; color: var(--texte); }
...
.remember-check {
  ...
  font-size: 13px; color: var(--texte-sec);
  margin-bottom: 20px; cursor: pointer; user-select: none;
}
.remember-check input { width: 15px; height: 15px; accent-color: var(--vert); cursor: pointer; }

.w-full { width: 100%; margin-top: 4px; padding: 13px; font-size: 15px; }
